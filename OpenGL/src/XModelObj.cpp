...
bool XModel::loadObjFromFile( const char *file, XModel &m) {
    FILE *fp;
    ...
    if (!vtx4Comp) {
        std::vector<float>::iterator src = m._positions.begin();
        std::vector<float>::iterator dst = m._positions.begin();
    ...
    if (!tex3Comp) {
        std::vector<float>::iterator src = m._texCoords.begin();
        std::vector<float>::iterator dst = m._texCoords.begin();
